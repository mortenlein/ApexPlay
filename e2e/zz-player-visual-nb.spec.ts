...
// `VISUAL_AUDIT=1 E2E_PORT=4104 npx playwright test e2e/zz-player-visual-nb.spec.ts`).
//
// The Norwegian twin of zz-player-visual.spec.ts: same states, same two viewports, with the
// locale cookie set to `nb`. It exists because Norwegian runs 10-30% longer than English
// ("Registration" → "Påmelding", "Leave team" → "Forlat laget"), and the place that breaks is
// the phone: a 390px-wide call board read across a loud hall. Every shot here is meant to be
// looked at, not asserted on.
//
...
const OUT = process.env.SHOT_DIR || 'test-results/player-shots-nb';
...
test('capture the player surfaces in Norwegian', async ({ browser }) => {
